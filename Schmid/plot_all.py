...
def plot_schmid_factors(infile, orient, outfile=None):
    ...
    for i, value in enumerate(unique_values):
        ...
        if (abs(y.values[0]) > thresh):         #  or x.values[0] > 0.4
    ...
    orient_formatted = " ".join(f"{x:.2f}" for x in orient)
    ax.set_title(f'<{orient_formatted}>-oriented ZnO micropillars', fontsize=12)
    # plt.legend()
