...
class Wurtzite:
    ...
    def _init_metric_tensors(self) -> None:
        ...
        # Build lattice vectors matrix for hexagonal system
        # a1 = a * (1, 0, 0)
        # a2 = a * (-1/2, sqrt(3)/2, 0)  
        # a3 = c * (0, 0, 1)
        ...
        self.lattice_vectors = np.array([
            [self.a, -self.a/2, 0],
            [0, self.a*np.sqrt(3)/2, 0],
            [0, 0, self.c]
        ])
        
        # For Miller-Bravais (4-index) system, we need a 4x3 transformation
        # The redundant index i = -(h+k) is handled separately
        self.lattice_vectors_4index = np.array([
            [self.a, -self.a/2, -self.a/2, 0],
            [0, self.a*np.sqrt(3)/2, -self.a*np.sqrt(3)/2, 0],
            [0, 0, 0, self.c]
        ])
        
        # Direct metric tensors
        self.G = self.lattice_vectors.T @ self.lattice_vectors
        self.G_4index = self.lattice_vectors_4index.T @ self.lattice_vectors_4index
        
        # Reciprocal metric tensors == inverse metric tensors
        try:
            self.G_inv = np.linalg.inv(self.G)
            self.G_inv_4index = (2/(9*self.a**2)) * np.array([      # define by hand, math doesn't apply to 4 indices
                [2, -1, -1, 0],
                [-1, 2, -1, 0],
                [-1, -1, 2, 0],
        ...
        self.transform_matrix = self.G_inv @ self.lattice_vectors.T
    ...
    def dot_product(self, vec1: Union[str, List], vec2: Union[str, List]) -> float:
        ...
        return np.dot(v1, np.dot(self.G_4index, v2))
    ...
    def plane_normal(self, hkil: Union[str, List]) -> List:
        ...
        normal = np.dot(indices, self.G_inv_4index)
    ...
    def cartesian_to_fractional(self, cartesian_coords: np.ndarray) -> np.ndarray:
        ...
        Args:
            cartesian_coords: Nx3 array of Cartesian coordinates or single 3D vector
        
        Returns:
            Fractional coordinates in lattice space
        """
        coords = np.atleast_2d(cartesian_coords)
        if coords.shape[1] != 3:
            raise ValueError("Cartesian coordinates must have 3 components")
        
        # Apply transformation: f = G^(-1) @ M^T @ r
        fractional = (self.transform_matrix @ coords.T).T
    ...
    def fractional_to_lattice_indices(self, fractional_coords: np.ndarray, 
                                    wrap: bool = True) -> np.ndarray:
        """
        Convert fractional coordinates to lattice indices.
        
        Args:
            fractional_coords: Fractional coordinates
            wrap: Whether to wrap coordinates to [0, 1) range
        
        Returns:
            Integer lattice indices
        """
        coords = np.atleast_2d(fractional_coords)
    ...
    def cartesian_to_lattice_indices(self, cartesian_coords: np.ndarray, 
                                   wrap: bool = True) -> np.ndarray:
        """
        Direct conversion from Cartesian coordinates to lattice indices.
        
        Args:
            cartesian_coords: Cartesian coordinates
            wrap: Whether to wrap fractional coordinates before rounding
        
        Returns:
            Integer lattice indices
        """
        fractional = self.cartesian_to_fractional(cartesian_coords)
        return self.fractional_to_lattice_indices(fractional, wrap)
    
    def fractional_to_cartesian(self, fractional_coords: np.ndarray) -> np.ndarray:
        """
        Convert fractional coordinates back to Cartesian (for verification).
        ...
        """
        coords = np.atleast_2d(fractional_coords)
        cartesian = (self.lattice_vectors @ coords.T).T
